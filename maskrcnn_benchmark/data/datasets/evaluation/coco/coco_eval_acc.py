...
def do_coco_evaluation(
    ...
    for iou_type in iou_types:
        with tempfile.NamedTemporaryFile() as f:
            ...
            for catId in evaluate_label:
                catId = id_list[catId-1]
    ...
    for image_id, prediction in enumerate(predictions):
        ...
        for i in labels:
            if i > 80:
                ...
                pred_label = dataset.contiguous_category_id_to_json_id[i]
            mapped_labels.append(pred_label)  
        coco_results.extend(
            [
                {
    ...
    def update(self, coco_eval):
        ...
        if len(catIds) is 1:
            ...
            for idx, metric in enumerate(metrics):
                res[catIds[0]][metric] = round(s[idx],4)
        else:
            for idx, metric in enumerate(metrics):
                res[metric] = round(s[idx],4)
